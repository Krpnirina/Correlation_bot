...
def initialize_files():
    ...
    with open(CSV_FILE, mode='w', newline='') as file:
        ...
        writer.writerow([
            ...
            "daily_min", "daily_max", "volume_status", "action", "trade_id"
        ])
...
async def handle_signal(ws, symbol, price, level_type, strength, daily_min, daily_max, timestamp):
    ...
    with open(CSV_FILE, mode='a', newline='') as file:
        ...
        writer.writerow([
            ...
            daily_min, daily_max, vol_status, action, trade_id or ""
        ])
    ...
    if len(data["ticks"]) >= MIN_TICKS_REQUIRED:
        ...
        if price <= support_zone:
            ...
            if (strength >= STRONG_LEVEL_THRESHOLD and 
                ...
                await handle_signal(trade_ws, symbol, price, "SUPPORT", strength, daily_min, daily_max, timestamp)
            ...
            if (strength >= STRONG_LEVEL_THRESHOLD and 
                ...
                data["levels_printed"].add(key)
                data["signal_counts"] += 1
                await handle_signal(trade_ws, symbol, price, "RESISTANCE", strength, daily_min, daily_max, timestamp)
...
async def main():
    initialize_files()
    print("\033[96m" + "━"*70)
    print(f"● AUTOMATED VOLATILITY INDEX TRADING SYSTEM | {datetime.utcnow().strftime('%Y-%m-%d %H:%M UTC')}")
